"""
cmn/tools/tool/bedrock_converse_tools_tool_chart.py
====================================================
Tool: render_chart

Does NOT render the chart itself.
Returns a structured chart payload that the UI layer (ChartToolRenderer) renders.

The model should:
  1. Fetch data first (e.g. via sales_data tool)
  2. Call this tool with the data + chart configuration
  3. Set color_series to the column that identifies different series
...
class ChartBedrockConverseTool(AbstractBedrockConverseTool):
    ...
    def invoke(self, params, tool_args: dict = None) -> dict:
        """
        Validates and returns the chart payload.
        Actual rendering happens in ChartToolRenderer in the UI layer.
        """
        args = tool_args or {}
        ...
        return {
            ...
            "data":         args.get("data"),
            "color_series": args.get("color_series"),
        }